...
void Log::on_regBtn_clicked()
{
    ...
    if (!QID.exec()) {
        ...
        DB_Connect.close();
        QMessageBox::critical(this, "Ошибка", "Ошибка проверки логина в базе данных.");
        return;
    }
    ...
    if (QID.next()) {
        QSqlDatabase::database().rollback();
        DB_Connect.close();
        QMessageBox::warning(this, "Внимание", "Пользователь с таким логином уже зарегистрирован.");
        return;
    }
    ...
    //createDataBase(ui->lineEdit_2->text());
